(function(undefined) {
    ...
    function email(from, from_email, to, to_email, subject, body, body_html, attachments) {
        ...
        var masterEmailQuery =
              "-- query to pull necessary fields for an email \n" +
              "--   called with proxy mode. \n" +
              "-- \n" +
        ...
        var make_query = function(this_) {
            ...
            var sources = {
                '_from': Rdbhost.util.maybe_null(from),
                '_from_email': Rdbhost.util.maybe_null(from_email),
                '_to': Rdbhost.util.maybe_null(to),
                '_to_email': Rdbhost.util.maybe_null(to_email),
                '_subject': Rdbhost.util.maybe_null(subject),
                '_body': Rdbhost.util.maybe_null(body),
                '_body_html': Rdbhost.util.maybe_null(body_html),
...
(function(window) {
    ...
    var addApiKey = " \
     -- create and invoke temporary function to \n\
     --   add an API key to apikeys table. \n\
     -- \n\
     CREATE FUNCTION pg_temp.t(_service VARCHAR, _apikey VARCHAR, _acct_email VARCHAR) \n\
     RETURNS void \n\
         AS $$ \n\
     BEGIN \n\
    ...
    var createTemplateTable = " \n\
     -- create table for templates used in emails. \n\
     --  also insert a tiny email template 'welcome.tpl' \n\
     -- \n\
