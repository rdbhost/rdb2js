(function(undefined) {
    ...
    function fixed(val) {
        return new Rdbhost.util.Fixed_Wrapper(val);
    }
    function column(val) {
        return new Rdbhost.util.Column_Wrapper(val);
    }
    ...
    function email(from, from_email, to, to_email, subject, body, body_html, attachments) {
        ...
        var f = function(this_) {
            ...
            var sources = {
                ...
                '_query_key': new Rdbhost.util.Bare_Wrapper(query_key),
                '_q': new Rdbhost.util.Subquery_Wrapper(this_, this_.q),
            ...
            if (attachments && Object.keys(attachments).length) {
                ...
                if (attachments instanceof Rdbhost.util.Fixed_Wrapper) {
                    ...
                    sources['_attachmentnames'] =  new Rdbhost.util.Literal_Wrapper(attachmentnames);
                    sources['_attachmentbodies'] =  new Rdbhost.util.Literal_Wrapper(attachmentbodies);
                }
            }
            ...
            var tokenRe = new RegExp('\{\{(\\S+)\}\}'),
                q = masterEmailQuery,
                m, token_name, ipp, ippCall;

            while (m = tokenRe.exec(q)) {

                token_name = '_'+m[1];
                ippCall = sources[token_name];
                ipp = ippCall instanceof Rdbhost.util.Column_Wrapper
                    ? ippCall.inline_plus_params(QUERY_KEY)
                    : ippCall.inline_plus_params();
    ...
    function email_host(from, from_email, subject, body, body_html, attachments) {
        ...
        return email.call(this, from, from_email, fixed(host_name), fixed(host_email), subject, body, body_html, attachments);
    }
    ...
    function email_user(to, to_email, subject, body, body_html, attachments) {
        ...
        return email.call(this, fixed(host_name), fixed(host_email), to, to_email, subject, body, body_html, attachments);
    }
