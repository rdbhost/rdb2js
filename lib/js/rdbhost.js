...
(function (window, undefined) {
    ...
    var events = new EventEmitter(),
        ...
        requestId = 1,
        requestTracker = {},
        PAGE_URL = 'http://devdemos.noservercoding.com/rdb2js/lib/',
        partialCache = {},
        HOST, ACCT;
     ...
     */
    function show_form(formText, error, notice) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            // clear form
            form.querySelector("span.error").textContent = error || '';
            ...
            function onSubmit(evt) {
                ...
                if ( ! email.length )
                    form.querySelector("span.error").textContent = 'provide an email address';
                else if ( ! password.length )
                    form.querySelector("span.error").textContent = 'provide a password';
    ...
    /* save_authcode - saves super authcode to cache for reuse

        @param key - 'super' or 'preauth' or 'auth'
        @param authcode - the authcode to save
     */
    function save_authcode(key, superauthcode) {
        ...
        authorizationCache[key] = superauthcode;
    ...
    function handle_message(conn, role, json) {
        ...
        // handle live-reload content here, maybe
            ...
            // todo - handle broadcast messages here.
        }
...
*/
    ...
    /* get_super_auth - logs in to server, gets authcodes, provides super-authcode to caller
          via promise.
    ...
    /* _go_super - function that runs query as super, getting authcode interactively as necessary
     should be bound to a request object first, with raw_getter and partial_getter curried

     @param raw_getter - function that gets data, returning a promise
     @param partial_getter - function that gets an html partial page, returning a promise
     @param req_preparer - function that prepares request for sending to server

     final bound signature would have no parameters
     */
    function _go_super(raw_getter, partial_getter, req_preparer) {

        function _get_superauth() {

            var pgPromise = partial_getter.call(this_, 'super_auth');
            return pgPromise.then(function(form) {
                ...
                .then(function(eml_pass) {
                    ...
                    var pr_gs = get_super_auth(acct_number, email, password);

                    return pr_gs.catch(function(e) {

                        if ( e.message.substr(0,11) === 'bad input. ' )
                            return _get_superauth();
    ...
    /* _go_reader - function that runs query as reader
     ...
     final bound signature would have no parameters
     */
    function _go_reader(raw_getter, req_preparer) {
    ...
    /* _go_super - function that runs query as super, getting authcode interactively as necessary
     ...
     final bound signature would have no parameters
     */
    function _go_preauth(raw_getter, partial_getter, req_preparer) {

        function _get_preauth(error) {

            var pgPromise = partial_getter.call(this_, 'preauth_auth');
            return pgPromise.then(function(form) {

                    // append form to body and show it.
                    //  returns a promise, resolved by form submission, or rejected by cancel
                    return show_form(form, error);
                ...
                .then(function(eml_pass) {
                    ...
                    return pr_gs.catch(function(e) {
                        ...
                        if ( e.message.substr(0,11) === 'bad input. ' )
                            return _get_preauth('bad input format.');
                        if ( e.message.substr(0,11) === 'bad email/p' )
                            return _get_preauth('bad email or password');
    ...
    function Preauth() {
        ...
        req._go_websocket = _go_preauth.bind(req, req._raw_data_getter_ws, req._html_partial_getter_ws, req._prepare_json);
        req._go_https = _go_preauth.bind(req, req._raw_data_getter_http, req._html_partial_getter_ws, req._prepare_formdata);
        return req.init('preauth');
    }

    function Auth(password, authcode) {
        if ( !password ) throw new Error('provide authCode for auth role');
    ...
    function Super(authcode) {
        ...
        var req = Object.create(requestObjectPrototype);
        req._go_websocket = _go_super.bind(req, req._raw_data_getter_ws, req._html_partial_getter_ws, req._prepare_json);
        req._go_https = _go_super.bind(req, req._raw_data_getter_http, req._html_partial_getter_ws, req._prepare_formdata);
        if ( authcode )
    ...
    function Reader() {
        var req = Object.create(requestObjectPrototype);
        req._go_websocket = _go_reader.bind(req, req._raw_data_getter_ws, req._prepare_json);
        req._go_https = _go_reader.bind(req, req._raw_data_getter_http, req._prepare_formdata);
