...
*/
    ...
    var get_auth_auth = get_auth.bind(null, 'auth');
     ...
     */
    function _go_super(raw_getter, req_preparer) {

        function _get_superauth() {

            var pgPromise = this_._html_partial_getter_ws('super_auth');
    ...
    /* _go_preauth - function that runs query as preauth, getting super-authcode interactively as necessary
     should be bound to a request object first, with raw_getter and req_preparer curried
     ...
     final bound signature would have no parameters
     */
    function _go_preauth(raw_getter, req_preparer) {

        function _get_preauth(error) {

            var pgPromise = this_._html_partial_getter_ws('preauth_auth');
            return pgPromise.then(function(form) {
    ...
    function Preauth() {
        ...
        req._go_websocket = _go_preauth.bind(req, req._raw_data_getter_ws, req._prepare_json);
        req._go_https = _go_preauth.bind(req, req._raw_data_getter_http, req._prepare_formdata);
    ...
    function Super(authcode) {
        ...
        var req = Object.create(requestObjectPrototype);
        req._go_websocket = _go_super.bind(req, req._raw_data_getter_ws, req._prepare_json);
        req._go_https = _go_super.bind(req, req._raw_data_getter_http, req._prepare_formdata);
        if ( authcode )
