...
(function (window, undefined) {
    ...
    function inline_sql() {
        ...
        function get_data_for_inline() {
            ...
            return p.query(q).go().then(function(data) {
    ...
    var requestObjectPrototype = {
        ...
        init: function(role) {
            ...
            this.add_hook_pair(interpolate_wrapped, null, 'unwrap', 0.72);
        ...
        listen: function(channel) {
            ...
            this.add_hook_pair(add_listen.bind(null, channel, this.listen.idx), strip_listen, 'listen', 0.70);
        ...
        broadcast: function(channel, msg) {
            ...
            function add_broadcast(channel, i, this_) {
                ...
                var qParts = [this_.finalToSend.q,
                              'NOTIFY %(_broadcast_channel_{}), %(_broadcast_msg_{})s;'.replace(/\{\}/g, i)],
                    namedParams = _;

                namedParams['_broadcast_subquery_{}'.replace('{}', i)] = 0;
                namedParams['_broadcast_channel_{}'.replace('{}', i)] = channel;
                namedParams['_broadcast_msg_{}'.replace('{}', i)] = msg;

                var ipp = process_wrapped_vals(qParts.join(';\n'), [], namedParams, this_.QUERY_KEY);

                this_.finalToSend.q = ipp[0];

                if (typeof ipp[2] !== 'undefined')
                    _.extend(this_.finalToSend.namedParams, ipp[2]);
            }
