...
(function (window, undefined) {
    ...
    var __version__ = '2.3';
    var __release__ = '2.3.0';

    var DEBUG_SOCKET = true;
    var BROADCAST_ONLY = '/* broadcast only */';
    ...
    function Null_Marker() {

        this.inline_plus_params = function() {
            return (new Bare_Marker('NULL').inline_plus_params());
        }
    }
    function Regular_Marker(val) {
    ...
    function Subquery_Marker(q, args, namedParams, no_as) {

        this.inline_plus_params = function(query_key) {
            var ipp = process_wrapped_vals(q, args, namedParams);
    ...
    function process_wrapped_vals(q0, args0, namedParams0, QUERY_KEY) {
        ...
        q = q0.replace(namedParmsTokenRe, function(match, nm, pos) {
            ...
            if (ippCall && ippCall.inline_plus_params) {
                ...
                if (ippCall instanceof Rdbhost.util.Column_Marker)
                    ipp = ippCall.inline_plus_params(QUERY_KEY);

                else if (ippCall instanceof Rdbhost.util.Subquery_Marker) {
                    ipp = ippCall.inline_plus_params(QUERY_KEY);
                }
                else
                    ipp = ippCall.inline_plus_params();
            }
            else if (typeof ippCall !== 'undefined') {
                if (match === '%s') {
                    _tv = _.isArray(ippCall) ? encode_list(ippCall) : ippCall.toString();
                    ipp = [match, [_tv], undefined];
                }
                else {
                    _tv = _.isArray(ippCall) ? encode_list(ippCall) : ippCall.toString();
                    _t = {}; _t[nm] = _tv;
                ...
                if (match === '%s')
                    ...
                    throw new Error('name parameter {} not provided'.replace('{}', nm));
            }
    ...
    function connect(host, acct, template_path) {
        ...
        PAGE_URL = (acctGood ? template_path : acct) || 'https://'+HOST+'/vendor/rdbhost/'+__version__+'/lib/partials/';
    }
    ...
    function run_request_hooks() {
        ...
        var finalToSend = this.finalToSend, this_ = this;
    ...
    function createFinalToSend(this_) {

        var finalToSend = Object.create(this_);
        finalToSend.namedParams = _.extend({}, this_.namedParams || {});
        finalToSend.args = this_.args ? this_.args.slice(0) : [];
    ...
    function prepare_formdata(auth_cache) {
        ...
        this.finalToSend = createFinalToSend(this);
    ...
    var requestObjectPrototype = {
        ...
        init: function(role) {
            ...
            this.hooks = [];
            this.add_hook_pair(null, fix_record_sets, 'fixrecsets', 0.2);

            this.listen_idx = this.broadcast_idx = 0;
            ...
            this.add_hook_pair(interpolate_wrapped, null, 'unwrap', 0.73);
        ...
        /* add_hook_pair - adds a pair of hooks to hooks list
         ...
         @returns: undefined
         */
        add_hook_pair: function(request, response, label, order_key) {
            var o = {
                label: label || '-',
                order_key: order_key || 0.5,
        ...
        /* extract query - creates a SubQuery_Marker from request, and optionally, clears
            those parts of request

         @returns - Subquery_Marker with q, args, namedParams from this
         */
        extract_query: function() {

            return new Subquery_Marker(this.q, this.args, this.namedParams, /*no as*/ false );
        },
        ...
        listen: function(channels) {
            ...
            // ensure channel list is list of Bare_Marker marked items
            if (!_.isArray(channels))
                channels = [channels];
            channels = _.map(channels, function(chl) {
                if (chl.inline_plus_params || chl.indexOf('"')>-1)
                    throw new Error('channel must be simple name, without "');
                return new Bare_Marker(chl);
            });

            // update index tracker
            var idx = this.listen_idx = (this.listen_idx || 0);
            this.listen_idx += channels.length;
            ...
            function add_listen(channels, i, this_) {

                var q = ['%(_listen_subquery_{})s'.replace(/\{\}/g, i),
                         this_.finalToSend.q,
                        'COMMIT; BEGIN;'].join(';\n'),
                    namedParams = {};

                var subqParts = _.map(channels, function(chl, j) {
                    var si = i+j;
                    namedParams['_listen_channel_{}'.replace('{}', si)] = chl;
                    return 'LISTEN "%(_listen_channel_{})s"'.replace(/\{\}/g, si);
                });
                var subq = new Rdbhost.util.Subquery_Marker(subqParts.join(';'),
                                                            [], namedParams, 'no AS');

                this_.finalToSend.q = q;
                this_.finalToSend.namedParams['_listen_subquery_{}'.replace('{}', i)] = subq;
            }

            // Add this function to request-hooks, to be applied just
            //  before sending to server
            this.add_hook_pair(add_listen.bind(null, channels, idx), strip_listen.bind(null, channels.length),
                               'listen', 0.72);
        ...
        broadcast: function(channel, msg) {
            ...
            if (!channel.inline_plus_params)
                channel = new Fixed_Marker(channel);

            // avoid 'no query provided' exception on broadcast only requests
            if (!this.q)
                this.q = BROADCAST_ONLY;

            this.broadcast_idx = (this.broadcast_idx || 0)+1;

            function add_broadcast(channel, i, this_) {

                // remove broadcast-only null-query, if one was provided
                var q = this_.finalToSend.q.replace(new RegExp(BROADCAST_ONLY.replace(/\*/g, '\\*')+';*\n?\s*','ig'), '');

                var qParts = [q, '%(_broadcast_subquery_{})s'.replace('{}', i)],
                    broadcastQ = 'SELECT pg_notify(%(_broadcast_channel_{})s, %(_broadcast_msg_{})s);'.replace(/\{\}/g, i),
                    namedParams = {};

                if (!q) qParts.shift();

                namedParams['_broadcast_channel_{}'.replace('{}', i)] = channel;
                namedParams['_broadcast_msg_{}'.replace('{}', i)] = msg;

                var subq = new Subquery_Marker(broadcastQ, [], namedParams, /* no AS */ true);

                this_.finalToSend.q = qParts.join(';\n');
                this_.finalToSend.namedParams['_broadcast_subquery_{}'.replace('{}', i)] = subq;
            }

            // Add this function to request-hooks, to be applied just
            //  before sending to server
            this.add_hook_pair(add_broadcast.bind(null, channel, this.broadcast_idx), undefined, 'broadcast', 0.71);
        ...
        get_data:  function() {
            ...
            // this.finalToSend.namedParams = _.extend({}, this.namedParams || {});
            // this.finalToSend.args = this.args ? this.args.slice(0) : [];
    ...
    function get_confirmation() {
        ...
        var this_ = this,
            roleLu = {'p': 'preauth', 's': 'super', 'a': 'auth'},
            pgPromise = html_partial_getter_ws(roleLu[this_.role.substr(0,1)]+'_confirm'),
    ...
    window.Rdbhost = _.extend(window.Rdbhost || {}, {
        ...
        util: {
            ...
            Subquery_Marker: Subquery_Marker,

            fixed: function(v) {return new Fixed_Marker(v)},
            column: function(v) {return new Column_Marker(v)},
            regular: function(v) { return new Regular_Marker(v)},
            null: function(v) {return new Null_Marker()},

            maybe_null: function(v) { return (typeof v === 'undefined') ? new Null_Marker() : v }
        }
    });
