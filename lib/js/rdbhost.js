...
(function (window, undefined) {
    ...
    function wrapped_val(param) {
        ...
        if (typeof param === 'undefined') {
            return new Null_Marker(param);
        }
        else if ((param instanceof Fixed_Marker) || (param instanceof Column_Marker)) {
            return param;
        }
        return param;
    }
    ...
    var requestObjectPrototype = {
        ...
        broadcast: function(channel, msg) {
            ...
            function add_broadcast(channel, i, this_) {
                ...
                var qParts = [this_.finalToSend.q, '%(_broadcast_subquery_{})s'.replace('{}', i)],
                    broadcastQ = 'NOTIFY %(_broadcast_channel_{})s, %(_broadcast_msg_{})s;'.replace(/\{\}/g, i),
                    namedParams = {};

                if (!this_.finalToSend.q === BROADCAST_ONLY)
                    qParts.shift();
