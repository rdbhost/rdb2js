...
*/
            ...
            if (path.match(/\.js$/i)
                && (~document.body.innerHTML.indexOf(path)
                    || ~document.head.innerHTML.indexOf(path))) {
                return this.reloadPage();
            }
            else if ( pathsMatch(prep_path(path), prep_path(window.location.pathname)) ) {
                return this.reloadPage();
            }
            else {
                if ( pathsMatch(prep_path(path), prep_path(window.location.pathname+'/')) ) {
                    return this.reloadPage();
                }
            }
        };

        Reloader.prototype.reloadPage = _debounce(function() {
            return this.window.document.location.reload();
        }, 100);

        Reloader.prototype.reloadImages = function(path) {
            var expando, img, selector, styleNames, styleSheet, _i, _j, _k, _l,
                _len, _len1, _len2, _len3, _ref, _ref1, _ref2, _ref3, _results;
        ...
        Reloader.prototype.reloadStylesheet = function(path) {
            var imported, link, links, match, style, _i, _j, _k, _l,
                _len, _len1, _len2, _len3, _ref, _ref1;
            links = (function() {
...
*/
    ...
    /*  this integrates Reloader into the Rdbhost event system
    ...
    var reloader = new Reloader(window, console, Timer),
        options = {'liveCSS': true, 'liveImg': true},
        channelRole;


    Rdbhost.activate_reloader = function(conn) {

        Rdbhost.on('reload-request', function(channel, payload) {

            if ( channel !== 'rdbhost_ftp_channel:super' )
                return;

            if ( payload.substr(0, 10) !== 'SAVE FILE ' )
                return;

            var path = payload.substr(10).trim();
            if ( path.length === 0 )
                return;

            console.log('reload '+payload);
            reloader.reload(path, options);
        });

        function enable_listener() {
            channelRole = conn.role;
            return conn.query("NOTIFY \"rdbhost_ftp_channel:super\", '';")
                .listen('rdbhost_ftp_channel:super').get_data()
                .catch(function(e) {
                    console.log(e);
                })
                .then(function(d) {
                    console.log('livereload is loaded');
                    return d;
                });
        }
        Rdbhost.on('connecting', enable_listener);
