...
*/
    ...
    var reloader = new Reloader(window, window.console, Timer),
        options = {'liveCSS': true, 'liveImg': true};

    Rdbhost.on('connection-opened', function(channel, payload) {

        var conn;
        if ( ! channel in ['preauth', 'super', 'reader', 'auth'] )
            throw new Error('invalid connection channel name '+channel);

        conn = Rdbhost[channel]();

        conn.query("NOTIFY \"rdbhost_ftp_channel\", '';")
            .listen('rdbhost_ftp_channel').go()
            .catch(function(e) {
                console.log(e);
            });
    });
